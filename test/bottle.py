...
class DictProperty(object):
    ...
    def __get__(self, obj, cls): # __get__ 参数为 self, 包含描述符的对象, 该对象的类型，返回了被装饰函数的返回值
...
class BaseRequest(object):
    ...
    def forms(self):
        ...
        print('BaseRequest forms called')
        forms = FormsDict()
    ...
    def _body(self):
        print('BaseRequest _body called')
    ...
    def _get_body_string(self):
        ...
        print('BaseRequest _get_body_string called')
    ...
    def POST(self):
        ...
        args = dict(fp=self.body, environ=safe_env, keep_blank_values=True) # fp 是wsgi服务器生成的一个文件对象
        if py31:
...
def local_property(name=None):
    ...
    def fset(self, value): # self是调用该方法的object
        print('local_property set')
        print(type(self))
        print(value if len(str(value)) < 100 else '==\n==')
        ls.var = value
    def fdel(self):
        print(type(self))
        print('local_property del')
        del ls.var
...
class LocalRequest(BaseRequest):
    ...
    environ = local_property() # environ是object的变量不是类属性 property函数会将environ绑定到self.environ
...
def run(app=None, server='wsgiref', host='127.0.0.1', port=8080,
    ...
    """ Start a server instance. This method blocks until the server terminates.
     ...
     """
    if NORUN: return
    if reloader and not os.environ.get('BOTTLE_CHILD'): # main进程在env中添加了BOTTLE_CHILD,因此子进程不会在执行这个if下的代码
        try:
            lockfile = None
            fd, lockfile = tempfile.mkstemp(prefix='bottle.', suffix='.lock')
            os.close(fd) # We only need this file to exist. We never write to it
            while os.path.exists(lockfile):
                args = [sys.executable] + sys.argv # 终端完整命令
                ...
                while p.poll() is None: # Busy wait...
                    ...
                    time.sleep(interval)
                if p.poll() != 3: # 修改代码poll的状态为3 poll()方法返回子进程的退出状态
    ...
    try:
        ...
        if reloader:
            ...
            with bgcheck:
                server.run(app) # interupt_main keyboardException 传递到了server的run中，而不是当前线程
